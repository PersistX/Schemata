...
class RecordTests: XCTestCase {
    func test_properties_string() {
        let title = Book.record.properties["title"]!
    ...
    func test_propertiesForKeyPath_string() {
        let properties = Book.record.properties(for: \Book.title)
        XCTAssertEqual(properties, [Book.record.properties["title"]!])
    }
    
    func test_propertiesForKeyPath_toOne_string() {
        let properties = Book.record.properties(for: \Book.author.name)
        XCTAssertEqual(properties, [
            Book.record.properties["author"]!,
            Author.record.properties["name"]!,
        ])
    }
}
